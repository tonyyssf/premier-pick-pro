
-- Create a view that maps the existing fixtures table to the expected schema
CREATE OR REPLACE VIEW app_fixtures AS
SELECT 
  gen_random_uuid() as id,
  NULL::uuid as gameweek_id,  -- Will need to be mapped based on Round Number
  "Match Number"::text as match_number,
  "Round Number" as round_number,
  "Home Team" as home_team_name,
  "Away Team" as away_team_name,
  "Location" as venue,
  CASE 
    WHEN "Date" ~ '^\d{2}/\d{2}/\d{4}$' THEN 
      ("Date"::date + interval '15:00')::timestamp with time zone
    ELSE 
      now()::timestamp with time zone
  END as kickoff_time,
  CASE 
    WHEN "Result" IS NOT NULL AND "Result" != '' THEN 'finished'
    ELSE 'scheduled'
  END as status,
  NULL::integer as home_score,
  NULL::integer as away_score,
  NULL::uuid as home_team_id,
  NULL::uuid as away_team_id,
  now() as created_at,
  now() as updated_at
FROM fixtures;

-- Create a function to get fixtures with proper team mapping
CREATE OR REPLACE FUNCTION get_app_fixtures_for_gameweek(gw_id uuid)
RETURNS TABLE (
  id uuid,
  gameweek_id uuid,
  home_team_id uuid,
  away_team_id uuid,
  kickoff_time timestamp with time zone,
  status text,
  home_score integer,
  away_score integer,
  home_team_name text,
  home_team_short_name text,
  home_team_color text,
  away_team_name text,
  away_team_short_name text,
  away_team_color text
)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public', 'pg_catalog'
AS $$
BEGIN
  RETURN QUERY
  SELECT 
    gen_random_uuid() as id,
    gw_id as gameweek_id,
    ht.id as home_team_id,
    at.id as away_team_id,
    CASE 
      WHEN f."Date" ~ '^\d{2}/\d{2}/\d{4}$' THEN 
        (f."Date"::date + interval '15:00')::timestamp with time zone
      ELSE 
        now()::timestamp with time zone
    END as kickoff_time,
    CASE 
      WHEN f."Result" IS NOT NULL AND f."Result" != '' THEN 'finished'
      ELSE 'scheduled'
    END as status,
    NULL::integer as home_score,
    NULL::integer as away_score,
    f."Home Team" as home_team_name,
    ht.short_name as home_team_short_name,
    ht.team_color as home_team_color,
    f."Away Team" as away_team_name,
    at.short_name as away_team_short_name,
    at.team_color as away_team_color
  FROM fixtures f
  LEFT JOIN teams ht ON ht.name = f."Home Team"
  LEFT JOIN teams at ON at.name = f."Away Team"
  WHERE f."Round Number" = (
    SELECT number FROM gameweeks WHERE id = gw_id
  );
END;
$$;

-- Grant necessary permissions
GRANT SELECT ON app_fixtures TO authenticated;
GRANT EXECUTE ON FUNCTION get_app_fixtures_for_gameweek(uuid) TO authenticated;
